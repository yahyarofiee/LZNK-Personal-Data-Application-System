<x-layouts.app-dpo title="Ditolak">

<style>
    @import url('https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,500;0,9..40,600;1,9..40,400&family=DM+Serif+Display:ital@0;1&display=swap');

    .page-wrap {
        font-family: 'DM Sans', sans-serif;
        background: #eef0ec;
        min-height: 100vh;
        padding: 2.25rem 2rem 4rem;
    }

    /* ── Page header ── */
    .page-header {
        margin-bottom: 1.75rem;
        border-bottom: 1px solid #d4d9d5;
        padding-bottom: 1.25rem;
        display: flex;
        align-items: flex-end;
        justify-content: space-between;
        flex-wrap: wrap;
        gap: 1rem;
    }
    .page-header h2 {
        font-family: 'DM Serif Display', serif;
        font-size: 1.75rem;
        color: #1a1f1b;
        margin: 0 0 4px;
        letter-spacing: -0.01em;
    }
    .page-header p { font-size: 13px; color: #5a6860; margin: 0; }

    /* Alert strip */
    .alert-tolak {
        background: #fff5f5;
        border: 1px solid #fecaca;
        border-left: 4px solid #dc2626;
        border-radius: 3px;
        padding: 10px 1.25rem;
        margin-bottom: 1.5rem;
        display: flex;
        align-items: center;
        gap: 10px;
        font-size: 13px;
        color: #7f1d1d;
    }
    .alert-tolak svg { width: 15px; height: 15px; flex-shrink: 0; color: #dc2626; }

    /* ── Panel ── */
    .panel {
        background: #fff;
        border: 1px solid #d4d9d5;
        border-radius: 4px;
        overflow: hidden;
        box-shadow: 0 2px 16px rgba(26,92,42,0.07);
    }
    .panel-head {
        padding: 1.1rem 1.5rem;
        border-bottom: 1px solid #eef0ec;
        display: flex;
        align-items: center;
        justify-content: space-between;
    }
    .panel-head-title {
        font-size: 13.5px; font-weight: 600; color: #1a1f1b;
        display: flex; align-items: center; gap: 8px;
    }
    .panel-head-title::before {
        content: ''; width: 3px; height: 14px;
        background: #dc2626; border-radius: 2px; display: block;
    }
    .panel-count {
        font-size: 12px; color: #7f1d1d;
        background: #fee2e2; border: 1px solid #fecaca;
        border-radius: 2px; padding: 3px 10px;
        display: flex; align-items: center; gap: 5px;
    }
    .panel-count::before {
        content: ''; width: 5px; height: 5px;
        border-radius: 50%; background: #dc2626;
    }

    /* ── Table ── */
    .dpo-table { width: 100%; border-collapse: collapse; font-size: 13.5px; }
    .dpo-table thead tr { background: #f5f6f4; border-bottom: 1.5px solid #d4d9d5; }
    .dpo-table thead th {
        padding: 11px 1.25rem;
        text-align: left;
        font-size: 11px; font-weight: 600; color: #8a9490;
        text-transform: uppercase; letter-spacing: 0.06em; white-space: nowrap;
    }
    .dpo-table thead th.center { text-align: center; }
    .dpo-table tbody tr { border-bottom: 1px solid #f0f2f0; transition: background 0.1s; }
    .dpo-table tbody tr:last-child { border-bottom: none; }
    .dpo-table tbody tr:hover { background: #fff9f9; }
    .dpo-table tbody td { padding: 14px 1.25rem; vertical-align: middle; }
    .dpo-table tbody td.center { text-align: center; }

    .td-nama { font-size: 13.5px; font-weight: 500; color: #1a1f1b; }
    .td-nama-sub { font-size: 11.5px; color: #8a9490; margin-top: 2px; }

    /* Status badge */
    .badge-tolak {
        display: inline-flex; align-items: center; gap: 5px;
        padding: 4px 10px; border-radius: 2px;
        font-size: 11.5px; font-weight: 600;
        background: #fee2e2; color: #7f1d1d;
    }
    .badge-tolak::before {
        content: ''; width: 5px; height: 5px;
        border-radius: 50%; background: #dc2626;
    }

    /* Selesai label */
    .selesai-wrap {
        display: inline-flex;
        align-items: center;
        gap: 5px;
        font-size: 12px;
        color: #8a9490;
    }
    .selesai-wrap svg { width: 13px; height: 13px; color: #dc2626; }

    /* Empty state */
    .empty-state { padding: 4rem 2rem; text-align: center; }
    .empty-icon {
        width: 52px; height: 52px; background: #fee2e2;
        border-radius: 50%; display: flex; align-items: center;
        justify-content: center; margin: 0 auto 1rem; color: #dc2626;
    }
    .empty-icon svg { width: 22px; height: 22px; }
    .empty-state p { font-size: 14px; color: #5a6860; font-weight: 500; margin: 0 0 4px; }
    .empty-state span { font-size: 12.5px; color: #8a9490; }
</style>

<div class="page-wrap">

    <!-- Page Header -->
    <div class="page-header">
        <div>
            <h2>Ditolak</h2>
            <p>Senarai permohonan yang telah ditolak</p>
        </div>
    </div>

    @php $count = \App\Models\Permohonan::where('status','Ditolak')->count(); @endphp

    @if($count > 0)
    <div class="alert-tolak">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
        Sebanyak <strong>&nbsp;{{ $count }} permohonan&nbsp;</strong> telah ditolak. Pemohon boleh kemaskini dan hantar semula.
    </div>
    @endif

    <!-- Table Panel -->
    <div class="panel">
        <div class="panel-head">
            <span class="panel-head-title">Permohonan Ditolak</span>
            <span class="panel-count">{{ $count }} ditolak</span>
        </div>

        <table class="dpo-table">
            <thead>
                <tr>
                    <th>Pemohon</th>
                    <th>Kategori</th>
                    <th>Status</th>
                    <th class="center">Tindakan</th>
                </tr>
            </thead>
            <tbody>

            @forelse(\App\Models\Permohonan::where('status','Ditolak')->latest()->get() as $item)
            <tr>
                <td>
                    <div class="td-nama">{{ $item->nama }}</div>
                    <div class="td-nama-sub">{{ $item->created_at->format('d/m/Y') }} · {{ $item->created_at->diffForHumans() }}</div>
                </td>
                <td style="color:#1a1f1b;">{{ $item->jenis }}</td>
                <td><span class="badge-tolak">Ditolak</span></td>
                <td class="center">
                    <div class="selesai-wrap">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                        Selesai
                    </div>
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="4">
                    <div class="empty-state">
                        <div class="empty-icon">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
                        </div>
                        <p>Tiada permohonan ditolak</p>
                        <span>Belum ada permohonan yang ditolak setakat ini.</span>
                    </div>
                </td>
            </tr>
            @endforelse

            </tbody>
        </table>
    </div>

</div>

</x-layouts.app-dpo>